...
# %%
dataset_test = DATASET_DICT[dataset]("/home/hl-fury/mariarosaria.briglia/ddpm-torch/data", "test")
dataset_train = DATASET_DICT[dataset]("/home/hl-fury/mariarosaria.briglia/ddpm-torch/data", "train")
# %%
test_dataloader = torch.utils.data.DataLoader(dataset_test, batch_size=64, shuffle=True, num_workers=4)
train_dataloader = torch.utils.data.DataLoader(dataset_train, batch_size=64, shuffle=True, num_workers=4)
# %% 
import resnets
wr = resnets.wide_resnet28_10().to(device)
state_dict = torch.load('/home/hl-fury/mariarosaria.briglia/ddpm-torch/model_best.pth.tar')['state_dict']
...
#  %%
images =diffusion.p_sample(model, shape=shape, device=device, noise=torch.randn(shape, device=device)).cpu()
images_to_be_evaluated = (x * 127.5 + 127.5).round().clamp(0, 255).to(torch.uint8).permute(0, 2, 3, 1).numpy()
